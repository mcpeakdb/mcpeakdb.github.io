<script setup lang="ts">
import { computed } from 'vue';
import { type StandardCard, SUIT_COLORS } from '@/types/cards';
import { CARD_BACKS, type CardSize, type CardBack } from '@/constants/cardStyles';
import BaseCard from './BaseCard.vue';
import CardSuit from './CardSuit.vue';
import CornerNumber from './CornerNumber.vue';

interface Props {
  card: StandardCard;
  isFaceUp?: boolean;
  isInteractive?: boolean;
  isSelected?: boolean;
  size?: CardSize;
  cardBack?: CardBack;
  showAnimation?: boolean;
}

const props = withDefaults(defineProps<Props>(), {
  isFaceUp: false,
  isInteractive: false,
  isSelected: false,
  size: 'md',
  cardBack: 'red',
  showAnimation: true,
});

const emit = defineEmits<{
  click: [card: StandardCard];
  select: [card: StandardCard];
}>();

// Card type detection
const cardValue = computed(() =>
  Array.isArray(props.card.value) ? props.card.value[0] : props.card.value,
);

const isFaceCard = computed(() => ['J', 'Q', 'K'].includes(props.card.text));
const isAce = computed(() => props.card.text === 'A');
const isNumberCard = computed(() => !isFaceCard.value && !isAce.value);

// Styling
const suitColor = computed(() => SUIT_COLORS[props.card.suit]);
const cardClasses = computed(() => ({
  'text-red-500': suitColor.value === 'red',
  'text-gray-800': suitColor.value === 'black',
}));

// Card layout for number cards
const getSuitPositions = (value: number) => {
  const layouts: Record<number, Array<{ class: string; rotated?: boolean }>> = {
    2: [
      { class: 'top-4 left-1/2 -translate-x-1/2' },
      { class: 'bottom-4 left-1/2 -translate-x-1/2', rotated: true },
    ],
    3: [
      { class: 'top-4 left-1/2 -translate-x-1/2' },
      { class: 'top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2' },
      { class: 'bottom-4 left-1/2 -translate-x-1/2', rotated: true },
    ],
    4: [
      { class: 'top-4 left-4' },
      { class: 'top-4 right-4' },
      { class: 'bottom-4 left-4', rotated: true },
      { class: 'bottom-4 right-4', rotated: true },
    ],
    5: [
      { class: 'top-4 left-4' },
      { class: 'top-4 right-4' },
      { class: 'top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2' },
      { class: 'bottom-4 left-4', rotated: true },
      { class: 'bottom-4 right-4', rotated: true },
    ],
    6: [
      { class: 'top-4 left-4' },
      { class: 'top-4 right-4' },
      { class: 'top-1/2 left-4 -translate-y-1/2' },
      { class: 'top-1/2 right-4 -translate-y-1/2' },
      { class: 'bottom-4 left-4', rotated: true },
      { class: 'bottom-4 right-4', rotated: true },
    ],
    7: [
      { class: 'top-4 left-4' },
      { class: 'top-4 right-4' },
      { class: 'top-8 left-1/2 -translate-x-1/2' },
      { class: 'top-1/2 left-4 -translate-y-1/2' },
      { class: 'top-1/2 right-4 -translate-y-1/2' },
      { class: 'bottom-4 left-4', rotated: true },
      { class: 'bottom-4 right-4', rotated: true },
    ],
    8: [
      { class: 'top-4 left-4' },
      { class: 'top-4 right-4' },
      { class: 'top-8 left-1/2 -translate-x-1/2' },
      { class: 'top-1/2 left-4 -translate-y-1/2' },
      { class: 'top-1/2 right-4 -translate-y-1/2' },
      { class: 'bottom-8 left-1/2 -translate-x-1/2', rotated: true },
      { class: 'bottom-4 left-4', rotated: true },
      { class: 'bottom-4 right-4', rotated: true },
    ],
    9: [
      { class: 'top-4 left-4' },
      { class: 'top-4 right-4' },
      { class: 'top-8 left-4' },
      { class: 'top-8 right-4' },
      { class: 'top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2' },
      { class: 'bottom-8 left-4', rotated: true },
      { class: 'bottom-8 right-4', rotated: true },
      { class: 'bottom-4 left-4', rotated: true },
      { class: 'bottom-4 right-4', rotated: true },
    ],
    10: [
      { class: 'top-4 left-4' },
      { class: 'top-4 right-4' },
      { class: 'top-6 left-1/2 -translate-x-1/2' },
      { class: 'top-8 left-4' },
      { class: 'top-8 right-4' },
      { class: 'bottom-8 left-4', rotated: true },
      { class: 'bottom-8 right-4', rotated: true },
      { class: 'bottom-6 left-1/2 -translate-x-1/2', rotated: true },
      { class: 'bottom-4 left-4', rotated: true },
      { class: 'bottom-4 right-4', rotated: true },
    ],
  };

  return layouts[value] || [];
};

const handleCardClick = (card: StandardCard | undefined) => {
  if (!card) return;
  emit('click', card);
  emit('select', card);
};
</script>

<template>
  <BaseCard
    :is-face-up="isFaceUp"
    :is-interactive="isInteractive"
    :is-selected="isSelected"
    :size="size"
    :show-flip-animation="showAnimation"
    :card="card"
    @click="handleCardClick"
  >
    <template #front>
      <div class="bg-white w-full h-full p-2 relative" :class="cardClasses">
        <!-- Corner numbers -->
        <CornerNumber :card="card" position="top-left" :size="size === 'sm' ? 'xs' : 'sm'" />
        <CornerNumber :card="card" position="bottom-right" :size="size === 'sm' ? 'xs' : 'sm'" />

        <!-- Card content -->
        <div class="absolute inset-4 flex items-center justify-center">
          <!-- Face cards -->
          <div v-if="isFaceCard" class="text-6xl text-jacquard-24">
            {{ card.text }}
          </div>

          <!-- Aces -->
          <CardSuit
            v-else-if="isAce"
            :suit="card.suit"
            :size="card.suit === 'spade' ? 'xl' : 'lg'"
          />

          <!-- Number cards -->
          <div v-else-if="isNumberCard" class="relative w-full h-full">
            <CardSuit
              v-for="(position, index) in getSuitPositions(cardValue)"
              :key="`suit-${index}`"
              :suit="card.suit"
              size="md"
              :class="['absolute', position.class, { 'rotate-180': position.rotated }]"
            />
          </div>
        </div>
      </div>
    </template>

    <template #back>
      <div class="w-full h-full" :class="CARD_BACKS[cardBack]"></div>
    </template>
  </BaseCard>
</template>
